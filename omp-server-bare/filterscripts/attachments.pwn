...
{
    if(!strcmp(cmdtext, "/attachmentdirect", true))
    {
        // OPENMP_REF: Targeted RPC 113 -> RPC 116 fixture without RPC 61/62 dialogs.
        ...
        SendClientMessage(playerid, 0x66FF66FF,
                          "Direct attachment test: slot 0, PoliceCap1, head bone; edit started.");
        printf("[attachments] direct edit player=%d slot=0 model=18636 bone=2", playerid);
        return 1;
    }
    if(!strcmp(cmdtext, "/attachmentdirectoff", true))
    {
        RemovePlayerAttachedObject(playerid, 0);
        SendClientMessage(playerid, 0xFFCC66FF, "Direct attachment test: slot 0 removed.");
        printf("[attachments] direct remove player=%d slot=0", playerid);
        return 1;
    }
...
// OPENMP_REF: The modern callback prototype tags response as EDIT_RESPONSE.
