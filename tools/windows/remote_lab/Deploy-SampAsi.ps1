param(
    ...
    [ValidateSet("samp_probe.asi", "samp_re.asi")][string]$Name,
...
if (-not (Test-Path -LiteralPath $Candidate)) {
    throw "Candidate ASI not found: $Candidate"
}
...
$target = Join-Path ([string]$config.game_dir) $Name
...
if ($running.Count -gt 0) {
    throw "Refusing ASI deployment while GTA/SA-MP processes are running: $($running.name -join ', ')"
}
...
if (Test-Path -LiteralPath $target) {
    ...
    $backupName = "{0}_{1}_{2}.asi" -f $stem, $stamp, $targetHashBefore.Substring(0, 12)
...
if ((Get-SampLabHash -Path $temporary) -ne $candidateHash) {
    ...
    throw "Candidate hash changed while staging the ASI."
}
Move-Item -LiteralPath $temporary -Destination $target -Force

$targetHashAfter = Get-SampLabHash -Path $target
if ($targetHashAfter -ne $candidateHash) {
    throw "Installed ASI hash does not match the candidate hash. Backup: $backup"
}
