// STATIC_037 focused metadata export for the SA-MP 0.3.7-R5 RPC 176 path.
...
public class InspectActorSetPosition extends GhidraScript {
    private static final long IMAGE_BASE = 0x10000000L;

    private static final FocusFunction[] FOCUS_FUNCTIONS = new FocusFunction[] {
        new FocusFunction("rpc176_handler", 0x1001dad0L),
        new FocusFunction("cactor_set_position", 0x1009f040L),
        new FocusFunction("bitstream_view_constructor", 0x1001f840L),
        new FocusFunction("bitstream_read_bits", 0x1001f9b0L)
    };
    ...
    protected void run() throws Exception {
        ...
        if (args.length < 1) {
            throw new IllegalArgumentException(
                "usage: InspectActorSetPosition.java <output-dir>");
        }
    ...
    private void exportFocusMetadata() throws Exception {
        ...
        for (FocusFunction focus : FOCUS_FUNCTIONS) {
            ...
            while (iterator.hasNext() && !monitor.isCancelled()) {
                ...
                String instructionBytes = bytes(instruction);
                ...
                instructions.write(tsv(focus.label,
                    rva(function.getEntryPoint().getOffset()),
                    rva(instruction.getAddress().getOffset()), instructionBytes,
                ...
                for (Reference ref : instruction.getReferencesFrom()) {
                    ...
                    if (ref.getReferenceType().isCall()) {
                        calls.write(tsv(focus.label,
                            ...
                            instructionBytes, instruction.getMnemonicString(),
                        ...
                        data.write(tsv(focus.label,
                            ...
                            rva(instruction.getAddress().getOffset()),
                            instructionBytes, instruction.getMnemonicString(),
                            instruction.toString(), ref.getToAddress(),
                            rva(ref.getToAddress().getOffset()),
                            ref.getReferenceType()));
                        dataRefs++;
                    }
                ...
                if (instruction.getFlowType().isCall() &&
                    ...
                    calls.write(tsv(focus.label,
                        ...
                        rva(instruction.getAddress().getOffset()),
                        instructionBytes, instruction.getMnemonicString(),
                        instruction.toString(), "indirect", "", "INDIRECT"));
                    callOut++;
                }
                    ...
                    if (flows.length == 0) {
                        ...
                        for (Address target : flows) {
                            branches.write(tsv(focus.label,
                                ...
                                rva(target.getOffset()), fallthrough));
                        }
                    }
                    branchCount++;
                }
    ...
    private void exportFocusMemory() throws Exception {
        ...
        memory.write(tsv("rpc176_entry_window", rva(0x1001dad0L),
            bytes(toAddr(0x1001dad0L), 32)));
        memory.write(tsv("rpc176_registration_window", rva(0x1001e71eL),
            bytes(toAddr(0x1001e71eL), 20)));
        memory.write(tsv("rpc176_id_value", rva(0x100e625cL),
            bytes(toAddr(0x100e625cL), 4)));
        memory.write(tsv("set_position_entry_window", rva(0x1009f040L),
            bytes(toAddr(0x1009f040L), 32)));
        memory.write(tsv("bitstream_view_constructor_entry",
            rva(0x1001f840L), bytes(toAddr(0x1001f840L), 32)));
        memory.write(tsv("bitstream_read_bits_entry", rva(0x1001f9b0L),
            bytes(toAddr(0x1001f9b0L), 32)));
        memory.write(tsv("script_command_bridge_entry", rva(0x100b2310L),
            bytes(toAddr(0x100b2310L), 32)));
        memory.write(tsv("train_position_scm_descriptor", rva(0x100ec50cL),
            bytes(toAddr(0x100ec50cL), 8)));
