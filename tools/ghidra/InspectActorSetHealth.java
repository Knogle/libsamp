// STATIC_037 focused metadata export for the SA-MP 0.3.7-R5 RPC 178 path.
...
public class InspectActorSetHealth extends GhidraScript {
    private static final long IMAGE_BASE = 0x10000000L;

    private static final FocusFunction[] FOCUS_FUNCTIONS = new FocusFunction[] {
        new FocusFunction("rpc178_handler", 0x1001dbe0L),
        new FocusFunction("cactor_set_health", 0x1009c5d0L)
    };
    ...
    protected void run() throws Exception {
        ...
        if (args.length < 1) {
            throw new IllegalArgumentException("usage: InspectActorSetHealth.java <output-dir>");
        }
    ...
    private void exportIdentity() throws Exception {
        ...
        identity.write(tsv("compiler_spec_id", currentProgram.getCompilerSpec().getCompilerSpecID()));
        identity.close();
    }
    ...
    private void exportFocusMetadata() throws Exception {
        ...
        for (FocusFunction focus : FOCUS_FUNCTIONS) {
            ...
            if (function == null || function.getEntryPoint().getOffset() != focus.address) {
                functions.write(tsv(focus.label, "missing", rva(focus.address), 0, 0, 0, 0, 0, 0, "", ""));
                continue;
            }
            ...
            while (incomingRefs.hasNext() && !monitor.isCancelled()) {
                ...
                incoming.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
            ...
            while (iterator.hasNext() && !monitor.isCancelled()) {
                ...
                instructions.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
                    rva(instruction.getAddress().getOffset()), bytes, instruction.getMnemonicString(),
                ...
                for (Reference ref : instruction.getReferencesFrom()) {
                    ...
                    if (ref.getReferenceType().isCall()) {
                        calls.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
                        ...
                        data.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
                            rva(instruction.getAddress().getOffset()), bytes,
                ...
                if (instruction.getFlowType().isCall() && !emittedCallReference) {
                    calls.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
                        rva(instruction.getAddress().getOffset()), bytes,
                ...
                if (instruction.getFlowType().isJump() ||
                    ...
                    if (flows.length == 0) {
                        branches.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
                            rva(instruction.getAddress().getOffset()), bytes,
                            instruction.getMnemonicString(), instruction.toString(),
                            instruction.getFlowType(), "", fallthrough));
                    } else {
                        for (Address target : flows) {
                            branches.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
                                rva(instruction.getAddress().getOffset()), bytes,
                                instruction.getMnemonicString(), instruction.toString(),
                                instruction.getFlowType(), rva(target.getOffset()), fallthrough));
                        }
                    }
                    branchCount++;
                }
                ...
                for (int operandIndex = 0;
                    ...
                    for (Object object : instruction.getOpObjects(operandIndex)) {
                        if (object instanceof Scalar) {
                            ...
                            scalars.write(tsv(focus.label, rva(function.getEntryPoint().getOffset()),
            ...
            functions.write(tsv(focus.label, function.getName(),
                rva(function.getEntryPoint().getOffset()), function.getBody().getNumAddresses(),
                instructionCount, callIn, callOut, dataRefs, branchCount,
                function.getCallingConventionName(), bytes(function.getEntryPoint(), 16)));
        }
    ...
    private void exportFocusMemory() throws Exception {
        ...
        memory.write(tsv("health_threshold_float", rva(0x100e5940L),
            bytes(toAddr(0x100e5940L), 4)));
        memory.write(tsv("nonpositive_health_scm_descriptor", rva(0x100ec2a8L),
            bytes(toAddr(0x100ec2a8L), 8)));
        memory.write(tsv("netgame_global_initial_value", rva(0x1026eb94L),
            bytes(toAddr(0x1026eb94L), 4)));
        memory.close();
    }
